...
for r in range(5):
    p = 0
    a = 0
    for context, target in san_check:
        ...
        maximum, ratio = torch.max(log_probs, 1)

        predict = list(vocab)[ratio]

        if predict == target:
            print("Successfully predicted")
            print("Text: " + str(context)+ " " + "Objective: "+ target + " " + "Prediction: " + predict)
            p += 1
    ...
    print("Labels that have been identified correctly: " + str(p))
    print("Labels that have been identified incorrectly: " + str(a))

evaluation = [(("START", "The"), "philosopher"), (("START", "The"), "physicist"), (("START", "The"), "mathematician")]

for context, target in evaluation:
    ...
    if target == "mathematician":
        mathematician_target = model.embeddings( autograd.Variable( torch.LongTensor([word_to_ix["mathematician"]])))


    if target == "physicist":
        physicist_target = model.embeddings( autograd.Variable( torch.LongTensor([word_to_ix["physicist"]])))

    if target == "philosopher":
        philosopher_target = model.embeddings( autograd.Variable( torch.LongTensor([word_to_ix["philosopher"]])))


physicist_cos = F.cosine_similarity(physicist_target, mathematician_target)
philosopher_cos = F.cosine_similarity(philosopher_target, mathematician_target)

if physicist_cos > philosopher_cos:
    print("Most likely word prediction outcome: " + (list(vocab)[(list(vocab)).index("physicist")]))

else:

    print("Most likely word prediction outcome: " + (list(vocab)[(list(vocab)).index("philosopher")]))
